...
def main():
    ...
    if args.restore_from[:4] == 'http' :
        ...
        saved_state_dict = torch.hub.load_state_dict_from_url(args.restore_from, map_location=torch.device('cpu'))
    else:
        saved_state_dict = torch.load(args.restore_from, map_location=torch.device('cpu'))
    model.load_state_dict(saved_state_dict)

    model.eval()
    # No need to move model to GPU
    # model.cuda(gpu0)
    ...
    for index, batch in enumerate(testloader):
        ...
        size = size[0].numpy()
        # output = model(Variable(image, volatile=True))  # Remove .cuda(gpu0)
        with torch.no_grad():
